	...
	class MyhoursFiller
		...
		def month_filling
			...
			today.downto(1) do |day|
				...
				sunday =  day_tr.find('td:first-child').find('a').text
				# Compare if its the last td in the table, or, in other words, check if its saturday
				saturday =  day_tr.find('td:last-child').find('a').text
				if (sunday != " #{day.to_s} ") and (saturday != " #{day.to_s} ")
					day_filling if page.has_no_link?('Edit') and page.has_no_link?('Delete')
				else
